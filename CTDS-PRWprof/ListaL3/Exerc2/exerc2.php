...
<body>
 ...
 <?php
  ...
  //item b) Percorrer o vetor e mostrar os dados na página web no formato tabular - primeiramente, criamos o cabeçalho da tabela
  echo "<table>
  ...
  foreach($alunos as $nomeAluno => $notaAluno)
   ...
   //item c) Mostrar os dados do aluno com a maior nota no vetor - inicialmente, descobrimos a maior nota com a função max()
   $maiorNota = max($alunos);
   
   //por último, usamos a função que localiza, no vetor, a célula com a maior nota e retorna O ÍNDICE associado a esta célula com a mior nota. A função que faz isto é array_search()
   $nomeAlunoMaiorNota = array_search($maiorNota, $alunos);

   echo "<p> Dados do aluno com maior desempenho em PRW: <br>
             Nome = <span> $nomeAlunoMaiorNota </span> <br>
